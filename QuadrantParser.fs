...
let private findSideLength (lines: string list) : int =
    ...
    let size =
        if (width / size) * (height / size) < 9 then
            size / 2
        else
            size
...
let mapToCoordinates (side: string list) : Option<Set<int * int>> =
    if side[0][0] = ' ' then
        ...
        let side =
            side |> List.map (fun s -> s.ToCharArray() |> Array.toList |> List.indexed)

        let side = side |> List.indexed

        let side =
            side
            |> List.collect (fun (y, chars) -> chars |> List.map (fun (x, c) -> (x, y), c))
...
let addQuadrant (sides: Option<Set<int * int>> list list) =
    ...
    sides
    |> List.collect (fun (y, row) -> row |> List.indexed |> List.map (fun (x, pos) -> (x, y), pos))
...
let toQuadrantMap (sides: ((int * int) * Option<Set<int * int>>) list) : Map<int * int, Set<int * int>> =
    ...
    sides |> List.filter sideExists |> List.map deOption |> Map.ofList

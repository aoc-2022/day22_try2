...
let adjustedRight ((x,y):QPos) (rot:NorthEast) =
    match rot with
    | Rot0 -> (x-1,y)
    
let adjustedNorth ((x,y):QPos) (rot:NorthEast) = 
    match rot with 
    | Rot0 -> (x,y-1)
    
let adjustedSouth ((x,y):QPos) (rot:NorthEast) =
    match rot with
    | Rot0 -> (x,y+1)
...
let rec expand (cube:Cube) (quadrant:Quadrant) =
    printfn $"Expanding: {cube} {quadrant}"
    let rec expand1 (cube:Cube) (locations:Location list) : Cube =
        ...
        | Bottom::rest ->
            ...
            let cube = addSideIfPossible cube quadrant Left left (snd bottom.NE,fst bottom.NE)
            let cube = addSideIfPossible cube quadrant Right right (snd bottom.NE |> oppositeDirection, fst bottom.NE)
            let cube = addSideIfPossible cube quadrant Back back bottom.NE 
            printfn $"Bottom::front = {front}"
            expand1 cube rest
        | Back::rest ->
            let back: Side = cube.Sides[Bottom]
            let pos = back.Pos
            ...
            let cube = addSideIfPossible cube quadrant Right left back.NE 
            let cube = addSideIfPossible cube quadrant Left right (fst back.NE, snd back.NE |> oppositeDirection)
